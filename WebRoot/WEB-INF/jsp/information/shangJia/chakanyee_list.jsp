...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="shangjia/ordergetlistPage.do.do" method="post" name="userForm" id="userForm">
			<input type="hidden" value="${pd.user_shangjia_fid }" name="user_shangjia_fid" id="user_shangjia_fid"/>
			<table>
				<tr>
					<td>
					...
					<td><input class="span10 date-picker" name="searchDateStarts" id="searchDateStarts"  value="${pd.searchDateStarts}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:120px;" placeholder="按月份检索日期" title="按月份检索日期"/></td>
					<td><input class="span10 date-picker" name="searchDateStart" id="searchDateStart"  value="${pd.searchDateStart}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="开始日期" title="开始日期"/></td>
					<td><input class="span10 date-picker" name="searchDateEnd" name="searchDateEnd"  value="${pd.searchDateEnd}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="结束日期" title="结束日期"/></td>
					...
					<c:if test="${QX.cha == 1 }">
						<td style="vertical-align:top;"><button class="btn btn-mini btn-light" onclick="searchs();" title="检索"><i id="nav-search-icon" class="icon-search"></i></button></td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				<thead>
					<tr>
						...
						<th class='center'><i class="icon-time hidden-phone"></i>订单完成时间</th>
						<th class='center'>商品名称</th>
						<th class='center'>商品数量</th>
						<th class='center'>商品单价</th>
						<th class='center'>本单金额</th>
			...
			<tbody>
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty ChanKanYeElist}">
						<c:if test="${QX.cha == 1 }">
						<c:forEach items="${ChanKanYeElist}" var="var" varStatus="vs">
							...
							<tr>
								...
								<td class='center'><fmt:formatDate value="${var.qurysdTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
								<td class='center'>${var.goodsName}</td>
								<td class='center'>x${var.goodsNum}</td>
								<td class='center'>${var.presentPrice}</td>
								<td class='center'>${var.zongpresentPrice}</td>
								<td class='center'>${var.peisongfei}</td>
								<td style="width: 68px;">
									<a class='btn btn-mini' title="编辑" disabled="disabled" onclick="edit('${var.order_takeou_id }')" ><i class='icon-edit'></i></a>
					...
					<c:otherwise>
						<tr class="main_info">
							<td colspan="100" class="center">没有相关营业额数据</td>
						</tr>
					</c:otherwise>
				</c:choose>
	...
	<!-- 	<a href="#" id="btn-scroll-up" class="btn btn-small btn-inverse">
		...
		function del(tagID){
			bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>shangjia/SiOrderDelete.do?ids='+tagID;
					$.get(url,function(data){
						nextPage('${page.currentPage}');
					});
				}
		...
		function toExcel(){
			...
			var searchDateStarts = $("#searchDateStarts").val();
			var searchDateStart = $("#searchDateStart").val();
			var searchDateEnd = $("#searchDateEnd").val();
			var user_shangjia_fid = $("#user_shangjia_fid").val();
			/* var ROLE_ID = $("#role_id").val(); */
			window.location.href='<%=basePath%>shangjia/excel.do?searchName='+searchName+'&searchDateStart='+searchDateStart+'&searchDateEnd='+searchDateEnd+'&user_shangjia_fid='+user_shangjia_fid+'&searchDateStarts='+searchDateStarts;
		}
