...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索开始  -->
			<form action="shangjia/bankCardTixianListPage.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
	...
	function checkedNo(tagID){
		bootbox.confirm("确定要执行受理失败操作吗?", function(result) {
			if(result) {
				top.jzts();
				var url = '<%=basePath%>shangjia/tixianshuoli.do?ids='+tagID+'&num=0';
				$.get(url,function(data){
					nextPage('${page.currentPage}');
				});
			}
	...
	function checkedYes(tagID){
		bootbox.confirm("确定要执行确认受理操作吗?", function(result) {
			if(result) {
				top.jzts();
				var url = '<%=basePath%>shangjia/tixianshuoli.do?ids='+tagID+'&num=1';
				$.get(url,function(data){
					nextPage('${page.currentPage}');
				});
			}
		...
		bootbox.confirm(msg,function(result){
			if(result){
				...
				if(str==''){
					...
					$.ajax({
						type:'post',
						url:'<%=basePath%>shangjia/tixianshuoli.do',
						dataTyoe:'json',
						cache: false,
						data:{
							"ids":str,
