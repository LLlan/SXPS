...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索开始  -->
			<form action="h5sys/sysContactwmListPage.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty lianxiwmList}">
						<c:if test="${QX.cha == 1 }">
							<c:forEach items="${lianxiwmList}" var="var" varStatus="vs">
								<tr>
									<%-- <td class='center' style="width: 30px;">
										<label><input type='checkbox' name='ids' value="${var.contact_id}" /><span class="lbl"></span></label>
									</td> --%>
									<td class='center' style="width: 30px;">${vs.index+1}</td>
									<td class="center">${var.hotline}</td>
									<td class="center">${var.weChat}</td>
									<td style="width: 68px;">
										<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${var.contact_id }')" ><i class='icon-edit'></i></a>
										<a class='btn btn-mini btn-danger' title="删除"  onclick="del('${var.contact_id }')"><i class='icon-trash'></i></a>
									</td>
								</tr>
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>h5sys/lianxiwmAdd.do';
		 diag.Width = 400;
		 diag.Height = 300;
	...
	function edit(Id){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑";
		 diag.URL = '<%=basePath%>h5sys/golianxiwmEdit.do?Id='+Id;
		 diag.Width = 400;
		 diag.Height = 300;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	//删除
		function del(Id){
			bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>h5sys/deletelianxiwm.do?ids='+Id;
