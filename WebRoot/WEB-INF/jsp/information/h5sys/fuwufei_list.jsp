...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索开始  -->
			<form action="h5sys/fuwufeiListPage.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">平台服务费收取比例</th>
						<!--<th class="center">同城打车服务费收取比例</th>-->
				...
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty fuwufeiList}">
						<c:if test="${QX.cha == 1 }">
							<c:forEach items="${fuwufeiList}" var="var" varStatus="vs">
								<tr>
									<td class='center' style="width: 30px;">
										<label><input type='checkbox' name='ids' value="${var.fuwufei_id}" /><span class="lbl"></span></label>
									</td>
									<td class='center' style="width: 30px;">${vs.index+1}</td>
									<td class="center">${var.fuwubili}</td>
									<!--<td class="center">${var.fuwubili2}</td>-->
									<td style="width: 68px;">
										<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${var.fuwufei_id }')" ><i class='icon-edit'></i></a>
										<%-- <a class='btn btn-mini btn-danger' title="删除"  onclick="del('${var.fuwufei_id }')"><i class='icon-trash'></i></a> --%>
									</td>
								</tr>
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>h5sys/fuwufeiAdd.do';
		 diag.Width = 400;
		 diag.Height = 200;
	...
	function edit(Id){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑";
		 diag.URL = '<%=basePath%>h5sys/gofuwufeiEdit.do?Id='+Id;
		 diag.Width = 400;
		 diag.Height = 200;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
