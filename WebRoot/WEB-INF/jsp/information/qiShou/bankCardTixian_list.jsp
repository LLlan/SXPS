...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索开始  -->
			<form action="qishou/bankCardTixianListPage.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
						<span class="input-icon"> <input autocomplete="off" id="nav-search-input" type="text" name="searchName" value="${pd.searchName }" placeholder="车主真实姓名!" /> <i id="nav-search-icon"class="icon-search"></i> </span>
					</td>
							...
							<c:forEach items="${bankCardTixianList}" var="var" varStatus="vs">
								<tr>
									...
									<td class='center' style="width: 30px;">${vs.index+1}</td>
											...
											<td class="center"><fmt:formatDate value="${var.tixian_time}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
											<td class="center">
												<c:if test="${var.update_time=='' || var.update_time==null}">等待受理中...</c:if>
												<c:if test="${var.update_time!='' && var.update_time!=null}"><fmt:formatDate value="${var.update_time}" pattern="yyyy-MM-dd HH:mm:ss"/></c:if>
											</td>
											<td style="width: 128px;">
												<a class='btn btn-mini btn-danger' title="受理失败" onclick="checkedNo('${var.bank_card_tixian_id }','${var.user_qishou_fid }',this)" >受理失败</a>
												<a class='btn btn-mini btn-info' title="确认受理"  onclick="checkedYes('${var.bank_card_tixian_id }','${var.user_qishou_fid }',this)">确认受理</a>
											</td>
								</tr>
	...
	//审核受理失败
	function checkedNo(tagID,user_qishou_fid){
		bootbox.confirm("确定要执行受理失败操作吗?", function(result) {
			if(result) {
				top.jzts();
				var url = '<%=basePath%>qishou/tixianshuoli.do?ids='+tagID+'&user_qishou_fid='+user_qishou_fid+'&num=0';
				$.get(url,function(data){
					nextPage('${page.currentPage}');
				});
			}
		...
		//审核受理成功
	function checkedYes(tagID,user_qishou_fid){
		bootbox.confirm("确定要执行确认受理操作吗?", function(result) {
			if(result) {
				top.jzts();
				var url = '<%=basePath%>qishou/tixianshuoli.do?ids='+tagID+'&user_qishou_fid='+user_qishou_fid+'&num=1';
				$.get(url,function(data){
					nextPage('${page.currentPage}');
				});
			}
		...
		bootbox.confirm(msg,function(result){
			if(result){
				...
				if(str==''){
					...
					$.ajax({
						type:'post',
						url:'<%=basePath%>qishou/tixianshuoli.do',
						dataTyoe:'json',
						cache: false,
						data:{
							"ids":str,
