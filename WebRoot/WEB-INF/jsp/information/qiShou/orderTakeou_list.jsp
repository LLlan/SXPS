...
<div class="row-fluid">
	...
	<!-- 检索  -->
	<form action="<%=basePath%>/qishou/getOrderTakeoulistPage.do" method="post" name="userForm" id="userForm">
	<table>
		<tr>
			<td>
		...
		<c:choose>
			<c:when test="${not empty list}">
				...
				<tr>
					...
					<td class='center'>
						<c:if test="${var.order_remark=='' || var.order_remark==null}">该订单无备注信息</c:if>
						<c:if test="${var.order_remark!='' && var.order_remark!=null}">${var.order_remark}</c:if>
					</td>
					<td style="width: 68px;">
