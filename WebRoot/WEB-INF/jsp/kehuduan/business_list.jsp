...
<header>
    <div class="header">
        ...
        <div class="headRight">
            <a href="api/kehu/toSearch.do?lat=${lat}&lng=${lng}" class="searchA">输入关键字搜索商家</a>
        </div>
    </div>
</header>
<!--商家列表-->
...
<!-- 当列表为空时，暂无数据 -->
<c:if test="${empty businessList}"><%@include file="common.jsp" %></c:if>
<!--排序-->
<script>
    $(function() {
    	...
    	$.post('api/kehu/nearDistanceShangjia.do',
  	         {
  	        	...
  	        	var appendHtml="";
  	         	for(var i=0;i<data.businessList.length;i++){
  	         		appendHtml+= '<a href="api/kehu/shop.do?user_shangjia_id='+data.businessList[i].user_shangjia_id+'&mkID='+data.mkID+'&lat='+${lat}+'&lng='+${lng}+'" class="smShop">';
